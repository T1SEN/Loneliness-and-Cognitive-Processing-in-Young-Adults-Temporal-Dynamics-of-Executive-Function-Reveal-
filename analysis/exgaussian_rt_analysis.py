...
# Extract RT (assuming responseTime column exists)
if 'responseTime' in wcst_trials.columns:
    wcst_trials['rt_ms'] = wcst_trials['responseTime']
elif 'rt' in wcst_trials.columns:
    wcst_trials['rt_ms'] = wcst_trials['rt']
else:
    print("  ERROR: No RT column found. Using placeholder.")
    wcst_trials['rt_ms'] = np.random.lognormal(7, 0.5, len(wcst_trials))
...
exgauss_df['z_ucla'] = scaler.fit_transform(exgauss_df[['ucla_total']])
...
for gender, label in [(0, 'Female'), (1, 'Male')]:
    ...
    # Path a: UCLA → τ
    model_a = ols("tau ~ ucla_total", data=gender_data).fit()
    a = model_a.params['ucla_total']

    # Path b: τ → PE (controlling UCLA)
    model_b = ols("pe_rate ~ tau + ucla_total", data=gender_data).fit()
    b = model_b.params['tau']
    c_prime = model_b.params['ucla_total']
    ...
    for _ in range(1000):
        ...
        try:
            boot_a = ols("tau ~ ucla_total", data=boot_data).fit().params['ucla_total']
            boot_b = ols("pe_rate ~ tau + ucla_total", data=boot_data).fit().params['tau']
