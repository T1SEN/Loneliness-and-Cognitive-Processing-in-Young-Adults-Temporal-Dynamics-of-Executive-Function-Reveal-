...
def analyze_consistency(verbose: bool = True) -> pd.DataFrame:
    ...
    # Standardize EF metrics
    scaler = StandardScaler()
    for col in available_cols:
        if master[col].notna().sum() > 5:
            master[f'{col}_z'] = np.nan
            mask = master[col].notna()
            master.loc[mask, f'{col}_z'] = scaler.fit_transform(master.loc[mask, [col]])
...
def analyze_meta_control(verbose: bool = True) -> pd.DataFrame:
    ...
    df_pca['z_ucla'] = (df_pca['ucla_total'] - df_pca['ucla_total'].mean()) / df_pca['ucla_total'].std()
...
def analyze_threeway_interaction(verbose: bool = True) -> pd.DataFrame:
    ...
    for age_group in ['Younger', 'Older']:
        ...
        results.append({
            ...
            'ucla_beta': model.params.get('z_ucla', np.nan),
            'ucla_p': model.pvalues.get('z_ucla', np.nan),
            'interaction_beta': model.params.get(int_term, np.nan),
            'interaction_p': model.pvalues.get(int_term, np.nan)
        })

        if verbose:
            print(f"  {age_group} (N={len(subset)}): UCLA×Gender β={model.params.get(int_term, np.nan):.4f}, p={model.pvalues.get(int_term, np.nan):.4f}")

    # Three-way interaction term
    threeway_term = 'z_ucla:C(gender_male)[T.1]:age_centered'
    if threeway_term in model_3way.params:
        if verbose:
...
def analyze_anxiety_mask(verbose: bool = True) -> pd.DataFrame:
    ...
    formula_3way = 'pe_rate ~ z_ucla * C(gender_male) * z_anxiety + z_dass_dep + z_dass_str + z_age'
    model_3way = smf.ols(formula_3way, data=master).fit(cov_type='HC3')
    ...
    for anx_group, label in [(0, 'Low Anxiety'), (1, 'High Anxiety')]:
        ...
        formula = 'pe_rate ~ z_ucla * C(gender_male) + z_dass_dep + z_dass_str + z_age'
        model = smf.ols(formula, data=subset).fit(cov_type='HC3')
        ...
        if verbose:
            print(f"  {label} (N={len(subset)}): UCLA×Gender β={model.params.get(int_term, np.nan):.4f}, p={model.pvalues.get(int_term, np.nan):.4f}")
...
def analyze_extreme_groups(verbose: bool = True) -> pd.DataFrame:
    ...
    # FDR correction
    if len(results_df) > 0:
        _, p_adj, _, _ = multipletests(results_df['p'].values, method='fdr_bh')
...
def analyze_hidden_patterns(verbose: bool = True) -> pd.DataFrame:
    ...
    for task_name, metric in tasks.items():
        ...
        if verbose:
            print(f"  {task_name}: UCLA β={model.params.get('z_ucla', np.nan):.3f}, Interaction β={model.params.get(int_term, np.nan):.3f}")
...
def analyze_residual_ucla(verbose: bool = True) -> pd.DataFrame:
    ...
    master['z_ucla_resid'] = (master['ucla_residual'] - master['ucla_residual'].mean()) / master['ucla_residual'].std()
    ...
    for outcome in outcomes:
        ...
        int_term = 'z_ucla_resid:C(gender_male)[T.1]'
        results.append({
            'outcome': outcome,
            'n': len(valid),
...
def analyze_temporal_context(verbose: bool = True) -> pd.DataFrame:
    ...
    from analysis.utils.data_loader_utils import RESULTS_DIR
    try:
