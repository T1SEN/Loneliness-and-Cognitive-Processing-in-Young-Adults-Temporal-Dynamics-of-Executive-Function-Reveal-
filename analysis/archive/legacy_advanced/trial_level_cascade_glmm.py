...
# Define cascade as consecutive T2 errors; if none, fallback to any T2 error
prp_clean["t2_error"] = (~prp_clean["t2_correct"]).astype(int)
prp_clean["error_cascade_int"] = (prp_clean["t2_error_prev"] & prp_clean["t2_error"]).astype(int)
if prp_clean["error_cascade_int"].sum() == 0:
    prp_clean["error_cascade_int"] = prp_clean["t2_error"]
    print("  Cascade fallback: using any T2 error (no consecutive T2 errors found)")
...
try:
    formula_cascade_full = (
        "error_cascade_int ~ t2_error_prev + z_ucla + C(gender_male) + "
        "soa_scaled + z_dass_dep + z_dass_anx + z_dass_str"
    )
    formula_cascade_reduced = "error_cascade_int ~ t2_error_prev + z_ucla + C(gender_male) + soa_scaled"
    formula_cascade_min = "error_cascade_int ~ t2_error_prev + z_ucla + soa_scaled"
    cascade_formulas = [
        ("full", formula_cascade_full),
        ("reduced", formula_cascade_reduced),
        ("minimal", formula_cascade_min),
    ]
    ...
    result_cascade = None
    used_formula_label = None
    for label, formula_cascade in cascade_formulas:
        try:
            ...
            result_cascade = model_cascade.fit(method="lbfgs", maxiter=200, reml=False)
            used_formula_label = label
            break
        except Exception as e_inner:
            print(f"  Attempt '{label}' failed: {e_inner}")

    if result_cascade is None:
        raise RuntimeError("All cascade model attempts failed")

    print(f"  Cascade model converged using '{used_formula_label}' specification")
...
pes_results = None
try:
    prp_clean["t2_rt_z"] = zscore(prp_clean["t2_rt"])
    formula_pes_full = (
        "t2_rt_z ~ t2_error_prev * z_ucla * C(gender_male) + "
        "soa_scaled + z_dass_dep + z_dass_anx + z_dass_str"
        + (" + z_age" if "z_age" in prp_clean.columns else "")
    )
    formula_pes_reduced = (
        "t2_rt_z ~ t2_error_prev * z_ucla + C(gender_male) + soa_scaled "
        "+ z_dass_dep" + (" + z_age" if "z_age" in prp_clean.columns else "")
    )
    formula_pes_min = "t2_rt_z ~ t2_error_prev + z_ucla + C(gender_male) + soa_scaled"
    pes_formulas = [
        ("full", formula_pes_full),
        ("reduced", formula_pes_reduced),
        ("minimal", formula_pes_min),
    ]
    ...
    result_pes = None
    used_pes_formula = None
    for label, formula_pes in pes_formulas:
        try:
            ...
            result_pes = model_pes.fit(method="lbfgs", maxiter=200, reml=False)
            used_pes_formula = label
            break
        except Exception as e_inner:
            print(f"  Attempt '{label}' failed: {e_inner}")

    if result_pes is None:
        raise RuntimeError("All PES model attempts failed")

    print(f"  PES model converged using '{used_pes_formula}' specification")
