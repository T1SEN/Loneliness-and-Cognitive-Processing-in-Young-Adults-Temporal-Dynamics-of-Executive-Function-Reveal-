...
세 가지 인과 모형을 비교하여 외로움-스트레스-인지기능 관계의 구조를 탐색:
- Model 1 (C→L→S): 인지기능 → 외로움 → 스트레스
- Model 2 (C→S→L): 인지기능 → 스트레스 → 외로움
- Model 3 (L→S→C): 외로움 → 스트레스 → 인지기능

Usage:
    python -m analysis.advanced.path_stress_suite
    python -m analysis.advanced.path_stress_suite --analysis path_comparison
    python -m analysis.advanced.path_stress_suite --list

    from analysis.advanced import path_stress_suite
...
# Output directory
OUTPUT_DIR = ANALYSIS_OUTPUT_DIR / "path_stress_suite"
...
# =============================================================================
# MODEL SPECIFICATIONS - STRESS VERSION
# =============================================================================

# Model 1: C → L → S (Cognition → Loneliness → Stress)
# EF 저하 → 사회성 저하 → 외로움 → 스트레스
MODEL_1_CLS = """
...
# 외로움 → 인지기능 저하 → 스트레스 (이론적으로 비합리적이나 완전성을 위해 포함)
MODEL_4_LCS = """
...
def analyze_path_comparison(verbose: bool = True) -> pd.DataFrame:
    """
    Fit and compare four competing path models (STRESS version).
...
def generate_summary_report(verbose: bool = True):
    ...
    report_lines.append("  4) L -> C -> S: 외로움 -> 인지기능 -> 스트레스 (이론적으로 비합리적)")
    report_lines.append("")
...
def run(analysis: Optional[str] = None, verbose: bool = True, **kwargs) -> Dict[str, Any]:
    """
    Run path comparison suite (STRESS version).
