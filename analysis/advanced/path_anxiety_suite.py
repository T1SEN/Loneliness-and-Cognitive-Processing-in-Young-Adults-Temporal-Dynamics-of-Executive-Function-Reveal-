"""
Path Model Comparison Suite - ANXIETY Version
==============================================

경쟁 경로모형(Competing Path Models) 비교 분석 - 불안(Anxiety) 버전

세 가지 인과 모형을 비교하여 외로움-불안-인지기능 관계의 구조를 탐색:
- Model 1 (C→L→A): 인지기능 → 외로움 → 불안
- Model 2 (C→A→L): 인지기능 → 불안 → 외로움
- Model 3 (L→A→C): 외로움 → 불안 → 인지기능

Usage:
    python -m analysis.advanced.path_anxiety_suite
    python -m analysis.advanced.path_anxiety_suite --analysis path_comparison
    python -m analysis.advanced.path_anxiety_suite --list

    from analysis.advanced import path_anxiety_suite
    path_anxiety_suite.run()
...
try:
    ...
    print("[WARNING] semopy not available. Using OLS approximation.")
...
from analysis.preprocessing.standardization import safe_zscore
from analysis.utils.modeling import standardize_predictors

np.random.seed(42)

# Output directory
OUTPUT_DIR = ANALYSIS_OUTPUT_DIR / "path_anxiety_suite"
...
# =============================================================================
# MODEL SPECIFICATIONS - ANXIETY VERSION
# =============================================================================

# Model 1: C → L → A (Cognition → Loneliness → Anxiety)
# EF 저하 → 사회성 저하 → 외로움 → 불안
MODEL_1_CLA = """
z_ucla ~ c1*ef_composite + z_age + gender_male
z_dass_anx ~ d1*z_ucla + z_age + gender_male
"""

# Model 2: C → A → L (Cognition → Anxiety → Loneliness)
# EF 저하 → 불안 → 사회적 철수 → 외로움
MODEL_2_CAL = """
z_dass_anx ~ c2*ef_composite + z_age + gender_male
z_ucla ~ d2*z_dass_anx + z_age + gender_male
"""

# Model 3: L → A → C (Loneliness → Anxiety → Cognition)
# 외로움 → 불안 → 인지수행 저하 (정서 간섭 가설)
MODEL_3_LAC = """
z_dass_anx ~ c3*z_ucla + z_age + gender_male
ef_composite ~ d3*z_dass_anx + z_age + gender_male
"""

# Model 4: L → C → A (Loneliness → Cognition → Anxiety)
# 외로움 → 인지기능 저하 → 불안 (이론적으로 비합리적이나 완전성을 위해 포함)
MODEL_4_LCA = """
ef_composite ~ c4*z_ucla + z_age + gender_male
z_dass_anx ~ d4*ef_composite + z_age + gender_male
"""
...
MODEL_SPECS = {
    'Model1_CLA': {
        'spec': MODEL_1_CLA,
        'path': 'C → L → A',
        'description': '인지기능 → 외로움 → 불안',
        'exogenous': 'ef_composite',
        'mediator': 'z_ucla',
        'endogenous': 'z_dass_anx',
        'a_label': 'c1',
        'b_label': 'd1',
    },
    'Model2_CAL': {
...
def load_path_data() -> pd.DataFrame:
    ...
    pd.DataFrame
        ...
        - z_dass_anx: Standardized DASS anxiety
        - ef_composite: Mean of z-scored (pe_rate, stroop_interference, prp_bottleneck)
    ...
    required_cols = ['z_ucla', 'z_dass_anx', 'ef_composite', 'z_age', 'gender_male']
    ...
    print(f"[DATA] Loaded {len(df)} complete cases for path analysis (ANXIETY)")
...
@register_analysis(
    ...
    description="Compare 4 competing causal models: C→L→A, C→A→L, L→A→C, L→C→A"
)
def analyze_path_comparison(verbose: bool = True) -> pd.DataFrame:
    """
    Fit and compare four competing path models (ANXIETY version).

    Returns
    -------
    pd.DataFrame
        Comparison of fit indices across models
    """
    if verbose:
        print("\n" + "=" * 60)
        print("PATH MODEL COMPARISON - ANXIETY")
        print("=" * 60)
    ...
    if verbose:
        ...
        for _, row in comparison_df.iterrows():
            ...
            if pd.notna(row['cfi']):
                print(f"  CFI: {row['cfi']:.3f}")
                print(f"  RMSEA: {row['rmsea']:.3f}")
            ...
            print(f"  Indirect effect: {row['indirect']:.3f}" if pd.notna(row['indirect']) else "  Indirect: N/A")
...
@register_analysis(
    ...
    description="Bootstrap confidence intervals for indirect (mediated) effects"
)
def analyze_indirect_effects(verbose: bool = True, n_bootstrap: int = 5000) -> pd.DataFrame:
    ...
    -------
    pd.DataFrame
        Indirect effect estimates with 95% CIs
    """
    if verbose:
        print("\n" + "=" * 60)
        print("BOOTSTRAP INDIRECT EFFECTS - ANXIETY")
    ...
    for model_name, model_info in MODEL_SPECS.items():
        ...
        if verbose:
            ...
            print(f"  Indirect effect: {point_est:.4f} [{ci_lower:.4f}, {ci_upper:.4f}] {sig_str}")
...
def analyze_gender_moderated(verbose: bool = True) -> pd.DataFrame:
    ...
    Given the significant UCLA x Gender interaction, test whether
    causal structure differs by gender.

    Returns
    -------
    pd.DataFrame
        Gender-specific path coefficients
    """
    if verbose:
        print("\n" + "=" * 60)
        print("GENDER-MODERATED PATH ANALYSIS - ANXIETY")
        print("=" * 60)
    ...
    if len(df_male) < 30:
        print("[WARNING] Male sample size < 30. Results may be unstable.")
    if len(df_female) < 30:
        print("[WARNING] Female sample size < 30. Results may be unstable.")
    ...
    for gender, gender_df in [('male', df_male), ('female', df_female)]:
        for model_name, model_info in MODEL_SPECS.items():
            ...
            # Fit model without gender covariate
            spec_no_gender = model_info['spec'].replace('+ gender_male', '').replace('+ gender_male', '')
    ...
    if verbose:
        ...
        for model_name in MODEL_SPECS.keys():
            ...
            a_diff = abs(male_row['a_path'] - female_row['a_path'])
            b_diff = abs(male_row['b_path'] - female_row['b_path'])

            if a_diff > 0.2:
                print(f"  [!] Large a-path gender difference: delta = {a_diff:.3f}")
            if b_diff > 0.2:
                print(f"  [!] Large b-path gender difference: delta = {b_diff:.3f}")
...
@register_analysis(
    ...
    description="Bootstrap comparison of indirect effects between genders (Fisher's z test)"
)
def analyze_gender_indirect_comparison(verbose: bool = True, n_bootstrap: int = 2000) -> pd.DataFrame:
    """
    성별별 간접효과 부트스트랩 후 차이 유의성 평가.

    경로계수에 대해 남녀 차이가 통계적으로 유의한지 검정.
    ...
    if verbose:
        print("\n" + "=" * 60)
        print("GENDER INDIRECT EFFECT COMPARISON - ANXIETY")
        print(f"n_bootstrap = {n_bootstrap}")
        print("=" * 60)
    ...
    for model_name, model_info in MODEL_SPECS.items():
        ...
        def compute_gender_diff_stats(male_boots, female_boots, path_name):
            """Compute gender difference statistics using permutation test."""
            ...
            # SE of difference = sqrt(SE_male^2 + SE_female^2)
...
def plot_model_comparison(df: pd.DataFrame):
    ...
    if df['indirect'].notna().any():
        ...
        ax.set_title('Indirect Effect (a x b)', fontsize=12, fontweight='bold')
    ...
    plt.suptitle('Path Model Comparison - ANXIETY', fontsize=14, fontweight='bold')
    plt.tight_layout()
...
def plot_indirect_effects(df: pd.DataFrame):
    ...
    ax.set_title('Bootstrap 95% CIs for Indirect Effects - ANXIETY\n(Green = significant, Gray = non-significant)',
...
def plot_gender_comparison(df: pd.DataFrame):
    ...
    plt.suptitle('Gender-Stratified Path Coefficients - ANXIETY', fontsize=14, fontweight='bold')
    plt.tight_layout()
...
def plot_gender_indirect_comparison(df: pd.DataFrame):
    ...
    plt.suptitle('Bootstrap Gender Comparison - ANXIETY (Blue=Male, Orange=Female)', fontsize=14, fontweight='bold')
    plt.tight_layout()
...
def generate_summary_report(verbose: bool = True):
    ...
    report_lines.append("PATH MODEL COMPARISON SUMMARY - ANXIETY")
    report_lines.append("=" * 70)
    report_lines.append("")
    report_lines.append("연구 질문: 외로움, 불안, 인지기능 간의 인과 구조 중")
    report_lines.append("          어떤 모형이 데이터에 가장 적합한가?")
    report_lines.append("")
    report_lines.append("비교 모형:")
    report_lines.append("  1) C -> L -> A: 인지기능 -> 외로움 -> 불안")
    report_lines.append("  2) C -> A -> L: 인지기능 -> 불안 -> 외로움")
    report_lines.append("  3) L -> A -> C: 외로움 -> 불안 -> 인지기능")
    report_lines.append("  4) L -> C -> A: 외로움 -> 인지기능 -> 불안 (이론적으로 비합리적)")
    report_lines.append("")
...
def run(analysis: Optional[str] = None, verbose: bool = True, **kwargs) -> Dict[str, Any]:
    """
    Run path comparison suite (ANXIETY version).

    Parameters
    ----------
    analysis : str, optional
        Specific analysis to run ('path_comparison', 'indirect_effects', 'gender_moderated')
    ...
    if analysis:
        ...
        # Filter kwargs for specific analyses
        ...
        for name in order:
            if name in ANALYSES:
                ...
                if verbose:
                    print(f"\n[RUNNING] {spec.name}: {spec.description}")
                try:
                    # Pass n_bootstrap for bootstrap-based analyses
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Path Model Comparison Suite - ANXIETY')
