"""
Path Model Comparison Suite
===========================

경쟁 경로모형(Competing Path Models) 비교 분석

세 가지 인과 모형을 비교하여 외로움-우울-인지기능 관계의 구조를 탐색:
- Model 1 (C→L→D): 인지기능 → 외로움 → 우울
- Model 2 (C→D→L): 인지기능 → 우울 → 외로움
- Model 3 (L→D→C): 외로움 → 우울 → 인지기능

Usage:
    python -m analysis.advanced.path_comparison_suite
    python -m analysis.advanced.path_comparison_suite --analysis path_comparison
    python -m analysis.advanced.path_comparison_suite --list

    from analysis.advanced import path_comparison_suite
    path_comparison_suite.run()
...
# Output directory
OUTPUT_DIR = ANALYSIS_OUTPUT_DIR / "path_comparison_suite"
...
# Model 1: C → L → D (Cognition → Loneliness → Depression)
# EF 저하 → 사회성 저하 → 외로움 → 우울
MODEL_1_CLD = """
z_ucla ~ c1*ef_composite + z_age + gender_male
z_dass_dep ~ d1*z_ucla + z_age + gender_male
"""

# Model 2: C → D → L (Cognition → Depression → Loneliness)
# EF 저하 → 우울 → 사회적 철수 → 외로움
MODEL_2_CDL = """
z_dass_dep ~ c2*ef_composite + z_age + gender_male
z_ucla ~ d2*z_dass_dep + z_age + gender_male
"""

# Model 3: L → D → C (Loneliness → Depression → Cognition)
# 외로움 → 우울 → 인지수행 저하 (정서 간섭 가설)
MODEL_3_LDC = """
z_dass_dep ~ c3*z_ucla + z_age + gender_male
ef_composite ~ d3*z_dass_dep + z_age + gender_male
"""

# Model 4: L → C → D (Loneliness → Cognition → Depression)
# 외로움 → 인지기능 저하 → 우울 (이론적으로 비합리적이나 완전성을 위해 포함)
# 아버지 제안: "인지기능이 떨어져서 우울하다기보다는 그 반대가 맞겠지"
MODEL_4_LCD = """
ef_composite ~ c4*z_ucla + z_age + gender_male
z_dass_dep ~ d4*ef_composite + z_age + gender_male
"""
...
MODEL_SPECS = {
    'Model1_CLD': {
        'spec': MODEL_1_CLD,
        'path': 'C → L → D',
        'description': '인지기능 → 외로움 → 우울',
        'exogenous': 'ef_composite',
        'mediator': 'z_ucla',
        'endogenous': 'z_dass_dep',
        'a_label': 'c1',
        'b_label': 'd1',
    },
    'Model2_CDL': {
...
def load_path_data() -> pd.DataFrame:
    ...
    pd.DataFrame
        ...
        - z_dass_dep: Standardized DASS depression
    ...
    required_cols = ['z_ucla', 'z_dass_dep', 'ef_composite', 'z_age', 'gender_male']
    ...
    print(f"[DATA] Loaded {len(df)} complete cases for path analysis")
...
@register_analysis(
    ...
    description="Compare 3 competing causal models: C→L→D, C→D→L, L→D→C"
)
def analyze_path_comparison(verbose: bool = True) -> pd.DataFrame:
    """
    Fit and compare three competing path models.
    ...
    if verbose:
        print("\n" + "=" * 60)
        print("PATH MODEL COMPARISON")
        print("=" * 60)
        print("\nComparing 3 competing causal structures:")
...
def analyze_indirect_effects(verbose: bool = True, n_bootstrap: int = 5000) -> pd.DataFrame:
    ...
    if verbose:
        print("\n" + "=" * 60)
        print("BOOTSTRAP INDIRECT EFFECTS")
...
def analyze_gender_moderated(verbose: bool = True) -> pd.DataFrame:
    """
    Fit all 3 models separately for males and females.

    Given the significant UCLA × Gender interaction, test whether
    ...
    if verbose:
        print("\n" + "=" * 60)
        print("GENDER-MODERATED PATH ANALYSIS")
        print("=" * 60)
    ...
    if verbose:
        ...
        for model_name in MODEL_SPECS.keys():
            ...
            if a_diff > 0.2:
                print(f"  [!] Large a-path gender difference: Δ = {a_diff:.3f}")
            if b_diff > 0.2:
                print(f"  [!] Large b-path gender difference: Δ = {b_diff:.3f}")
...
def analyze_gender_indirect_comparison(verbose: bool = True, n_bootstrap: int = 2000) -> pd.DataFrame:
    ...
    특히 Model 1의 a-path (EF → UCLA)에서 남녀 방향이 반대인 것이 유의한지 확인.
    ...
    if verbose:
        print("\n" + "=" * 60)
        print("GENDER INDIRECT EFFECT COMPARISON")
        print(f"n_bootstrap = {n_bootstrap}")
        print("=" * 60)
...
def plot_indirect_effects(df: pd.DataFrame):
    ...
    ax.set_title('Bootstrap 95% CIs for Indirect Effects\n(Green = significant, Gray = non-significant)',
...
def plot_gender_comparison(df: pd.DataFrame):
    ...
    plt.suptitle('Gender-Stratified Path Coefficients', fontsize=14, fontweight='bold')
    plt.tight_layout()
...
def plot_gender_indirect_comparison(df: pd.DataFrame):
    ...
    # Filter only a-path and indirect for Model 1 (most interesting)
    ...
    plt.suptitle('Bootstrap Gender Comparison (Blue=Male, Orange=Female)', fontsize=14, fontweight='bold')
    plt.tight_layout()
...
def generate_summary_report(verbose: bool = True):
    ...
    report_lines.append("PATH MODEL COMPARISON SUMMARY")
    report_lines.append("=" * 70)
    report_lines.append("")
    report_lines.append("연구 질문: 외로움, 우울, 인지기능 간의 인과 구조 중")
    report_lines.append("          어떤 모형이 데이터에 가장 적합한가?")
    report_lines.append("")
    report_lines.append("비교 모형:")
    report_lines.append("  1) C → L → D: 인지기능 → 외로움 → 우울")
    report_lines.append("  2) C → D → L: 인지기능 → 우울 → 외로움")
    report_lines.append("  3) L → D → C: 외로움 → 우울 → 인지기능")
    report_lines.append("  4) L → C → D: 외로움 → 인지기능 → 우울 (이론적으로 비합리적)")
    report_lines.append("")
    ...
    try:
        ...
        gender = pd.read_csv(OUTPUT_DIR / "gender_paths.csv")
        ...
        report_lines.append("- 세 모형은 관찰적으로 동치 - 적합도 차이는 상대적 해석만 가능")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Path Model Comparison Suite')
