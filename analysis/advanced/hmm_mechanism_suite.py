"""
HMM Lapse Mechanism Suite
=========================

Deep analysis of Hidden Markov Model attentional states with proper DASS control.
...
4. Bayesian HMM for uncertainty quantification

Research Question:
------------------
Is there ANY unique UCLA effect on attentional dynamics beyond DASS confounding?
...
Usage:
    python -m analysis.advanced.hmm_mechanism_suite
    python -m analysis.advanced.hmm_mechanism_suite --analysis dass_controlled_hmm
    python -m analysis.advanced.hmm_mechanism_suite --list
...
# Output directory
OUTPUT_DIR = ANALYSIS_OUTPUT_DIR / "hmm_mechanism"
...
def load_hmm_data() -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # Load WCST trials (most trials per participant)
        ...
        try:
            ...
            for term in ['z_ucla', 'C(gender_male)[T.1]', 'z_ucla:C(gender_male)[T.1]']:
                if term in model.params:
                    ...
                    if verbose:
                        sig = "*" if p < 0.05 else ""
                        print(f"    {label} ~ {term_label}: β={beta:.4f}, SE={se:.4f}, p={p:.4f}{sig}")
        ...
        for metric in ['lapse_occupancy', 'trans_to_focus']:
            ...
            try:
                ...
                if 'z_ucla' in model.params:
                    ...
                    if verbose:
                        sig = "*" if p < 0.05 else ""
                        print(f"    {metric} ~ UCLA: β={beta:.4f}, p={p:.4f}{sig}")
...
def analyze_mediation_pathway(verbose: bool = True) -> pd.DataFrame:
    ...
    if verbose:
        print(f"\n  Path a (UCLA -> DASS): β={a_coef:.3f}, p={a_p:.4f}")
    ...
    if verbose:
        print(f"  Path b (DASS -> Lapse | UCLA): β={b_coef:.3f}, p={b_p:.4f}")
    ...
    if verbose:
        print(f"  Path c (Lapse -> PE | DASS, UCLA): β={c_coef:.3f}, p={c_p:.4f}")
        ...
        try:
            ...
            for term in ['z_ucla', 'z_ucla:C(gender_male)[T.1]']:
                if term in model.params:
                    ...
                    if verbose:
                        sig = "*" if p < 0.05 else ""
                        print(f"    {metric} ~ {label}: β={beta:.3f}, p={p:.4f}{sig}")
...
def run(analysis: Optional[str] = None, verbose: bool = True) -> Dict[str, pd.DataFrame]:
    ...
    if verbose:
        print("=" * 70)
        print("HMM LAPSE MECHANISM SUITE")
        print("=" * 70)
    ...
    if verbose:
        print("\n" + "=" * 70)
        print("HMM MECHANISM SUITE COMPLETE")
...
def list_analyses():
    ...
    print("\nAvailable HMM Mechanism Analyses:")
    print("-" * 60)
    for name, spec in ANALYSES.items():
        print(f"  {name}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="HMM Lapse Mechanism Suite")
