...
analysis_df = master.dropna(subset=base_cols).copy()
...
analysis_df['z_ucla'] = zscore(analysis_df['ucla_total'])
analysis_df['z_dass_dep'] = zscore(analysis_df['dass_depression'])
analysis_df['z_dass_anx'] = zscore(analysis_df['dass_anxiety'])
analysis_df['z_dass_stress'] = zscore(analysis_df['dass_stress'])
analysis_df['z_age'] = zscore(analysis_df['age'])

print(f"\n샘플 크기: N = {len(analysis_df)}")
print(f"남성: {analysis_df['gender_male'].sum()}명 ({analysis_df['gender_male'].sum()/len(analysis_df)*100:.1f}%)")
print(f"여성: {(1-analysis_df['gender_male']).sum()}명 ({(1-analysis_df['gender_male']).sum()/len(analysis_df)*100:.1f}%)")
...
for ef_var, ef_label in [
    ...
    data = analysis_df.dropna(subset=[ef_var]).copy()
    if len(data) < 30:
...
for gender_val, gender_label in [(0, 'Female'), (1, 'Male')]:
    ...
    for ef_var, ef_label in [
        ...
        valid = subset.dropna(subset=[ef_var])
        if len(valid) < 5:
            print(f"  {ef_label}: insufficient data")
            continue
        r, p = pearsonr(valid['ucla_total'], valid[ef_var])
...
for ef_var, ef_label in [
    ...
    if len(data) < 30:
        print("  Skipped - insufficient data")
        continue
    model_obs = smf.ols(formula, data=data).fit()
    ...
    for i in range(n_perm):
        shuffled = data.copy()
        shuffled['gender_male'] = RNG.permutation(shuffled['gender_male'].values)
...
for ef_var, ef_label in [
    ...
    ci_lower, ci_upper, boot_dist = bootstrap_interaction(data, formula, n_boot=5000)
...
for ef_var, ef_label in [
    ...
    data['ucla_centered'] = data['ucla_total'] - data['ucla_total'].mean()

    formula_age = f"{ef_var} ~ ucla_centered * age_centered + gender_male + z_dass_dep + z_dass_anx + z_dass_stress"
    model_age = smf.ols(formula_age, data=data).fit()
...
if not df_permutation.empty:
    ...
    print(f"\n1. ?? ?? ?? ????:")
    print(f"   ??: {most_sig['outcome']}")
    print(f"   ???? ??: {most_sig['observed_interaction']:.4f}")
    print(f"   Permutation p-value: {most_sig['p_permutation']:.4f}")
else:
    print("\nPermutation ??? ?? ?? ??")

wcst_slope_rows = df_simple_slopes[df_simple_slopes['outcome'] == 'WCST Perseverative Error Rate']
wcst_boot_rows = df_bootstrap[df_bootstrap['outcome'] == 'WCST']

if not wcst_slope_rows.empty and not wcst_boot_rows.empty:
    wcst_row_slope = wcst_slope_rows.iloc[0]
    wcst_row_boot = wcst_boot_rows.iloc[0]

    print(f"\n2. WCST ?? ??:")
    print(f"   ?? slope: ? = {wcst_row_slope['beta_female']:.4f}, p = {wcst_row_slope['p_female']:.4f}")
    print(f"   ?? slope: ? = {wcst_row_slope['beta_male']:.4f}, p = {wcst_row_slope['p_male']:.4f}")
    print(f"   Bootstrap 95% CI: [{wcst_row_boot['boot_ci_lower']:.4f}, {wcst_row_boot['boot_ci_upper']:.4f}]")

    if wcst_row_boot['excludes_zero']:
        print(f"   *** Bootstrap CI? 0 ? ?? -> ???? ???? ***")
    else:
        print(f"   Bootstrap CI? 0 ?? -> ???? ???")
else:
    print("\n2. WCST ??? ??? ??? ??? ????.")
