...
외로움-EF 관계에서 미약한 신호를 정밀하게 탐지 - 간소화 버전
"""
...
# Load master dataset (already preprocessed)
master = pd.read_csv(OUTPUT_DIR / "master_dataset.csv")

# Rename pe_rate to perseverative_error_rate for consistency
...
# =============================================================================
# 4. NON-LINEAR RELATIONSHIP TESTS
# =============================================================================
...
for ef_var, ef_label in [
    ...
    if abs(ols_coef) > 0.001 and abs(robust_coef - ols_coef) / abs(ols_coef) > 0.2:
    ...
    robust_results.append({
        ...
        'percent_change': (robust_coef - ols_coef) / abs(ols_coef) * 100 if abs(ols_coef) > 0.001 else np.nan
    })
...
print("\n" + "=" * 80)
print("SECTION 5: PARTIAL CORRELATIONS")
print("=" * 80)
...
print("\n" + "=" * 80)
print("SECTION 8: BOOTSTRAP CONFIDENCE INTERVALS")
print("=" * 80)
