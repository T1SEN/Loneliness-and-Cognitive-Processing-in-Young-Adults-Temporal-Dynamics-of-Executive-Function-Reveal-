...
from ..constants import PRP_RT_MIN, PRP_RT_MAX, RAW_DIR, get_results_dir
from ..surveys import get_survey_valid_participants, SurveyQCCriteria
from .participant_filters import get_prp_valid_participants, PRPQCCriteria
        ...
        if filename == "4a_prp_trials.csv":
            ...
            for timeout_col in ("t1_timeout", "t2_timeout"):
                if timeout_col in df_filtered.columns:
                    ...
                    df_filtered[timeout_col] = False

            df_filtered["is_timeout"] = df_filtered["t1_timeout"] | df_filtered["t2_timeout"]
            ...
            if "response_order" in df_filtered.columns:
                ...
                df_filtered["order_norm"] = df_filtered["response_order"].apply(_norm_order)
                df_filtered["is_order_valid"] = df_filtered["order_norm"] == "t1_t2"
            else:
                df_filtered["order_norm"] = None
                df_filtered["is_order_valid"] = True
            if "t2_pressed_while_t1_pending" in df_filtered.columns:
                ...
                df_filtered["is_pending_valid"] = ~df_filtered["t2_pressed_while_t1_pending"]
            else:
                df_filtered["t2_pressed_while_t1_pending"] = False
                df_filtered["is_pending_valid"] = True

            df_filtered["is_structurally_valid"] = (
                df_filtered["is_order_valid"] & df_filtered["is_pending_valid"]
            )

            df_filtered["is_t1_rt_valid"] = df_filtered["t1_rt_ms"].notna() & df_filtered["t1_rt_ms"].between(
                PRP_RT_MIN, PRP_RT_MAX
            )
            df_filtered["is_t2_rt_valid"] = df_filtered[rt_col].notna() & df_filtered[rt_col].between(
                PRP_RT_MIN, PRP_RT_MAX
            )
            df_filtered["is_rt_valid"] = (
                df_filtered["is_t1_rt_valid"]
                & df_filtered["is_t2_rt_valid"]
            )
