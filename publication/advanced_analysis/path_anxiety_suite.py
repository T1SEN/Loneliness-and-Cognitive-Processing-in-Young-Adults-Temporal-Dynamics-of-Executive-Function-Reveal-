...
네 가지 인과 모형을 비교하여 외로움-불안-인지기능 관계의 구조를 탐색:
- Model 1 (C→L→A): 인지기능 → 외로움 → 불안
- Model 2 (C→A→L): 인지기능 → 불안 → 외로움
- Model 3 (L→A→C): 외로움 → 불안 → 인지기능
- Model 4 (L→C→A): 외로움 → 인지기능 → 불안

Usage:
    python -m publication.advanced_analysis.path_anxiety_suite
    python -m publication.advanced_analysis.path_anxiety_suite --analysis path_comparison
    python -m publication.advanced_analysis.path_anxiety_suite --list

    from publication.advanced_analysis import path_anxiety_suite
...
# Output directory
OUTPUT_DIR = BASE_OUTPUT / "path_anxiety"
...
def analyze_gender_indirect_comparison(verbose: bool = True, n_bootstrap: int = 2000) -> pd.DataFrame:
    """
    성별별 간접효과 부트스트랩 후 차이 유의성 평가 (ANXIETY 버전).
...
def generate_summary_report(verbose: bool = True):
    ...
    report_lines.append("  4) L → C → A: 외로움 → 인지기능 → 불안")
    report_lines.append("")
...
def run(analysis: Optional[str] = None, verbose: bool = True, **kwargs) -> Dict[str, Any]:
    """Run path comparison suite (Anxiety)."""
    results = {}
