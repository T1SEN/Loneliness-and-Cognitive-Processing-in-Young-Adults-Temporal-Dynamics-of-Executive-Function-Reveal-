"""
Path Model Comparison Suite - DEPRESSION Version
=================================================

경쟁 경로모형(Competing Path Models) 비교 분석 - 우울(Depression) 버전

네 가지 인과 모형을 비교하여 외로움-우울-인지기능 관계의 구조를 탐색:
- Model 1 (C→L→D): 인지기능 → 외로움 → 우울
- Model 2 (C→D→L): 인지기능 → 우울 → 외로움
- Model 3 (L→D→C): 외로움 → 우울 → 인지기능
- Model 4 (L→C→D): 외로움 → 인지기능 → 우울

Usage:
    python -m publication.advanced_analysis.path_depression_suite
    python -m publication.advanced_analysis.path_depression_suite --analysis path_comparison
    python -m publication.advanced_analysis.path_depression_suite --list

    from publication.advanced_analysis import path_depression_suite
    path_depression_suite.run()
...
# Output directory
OUTPUT_DIR = BASE_OUTPUT / "path_depression"
...
@register_analysis(
    ...
    description="Compare 4 competing causal models: C→L→D, C→D→L, L→D→C, L→C→D"
)
...
def generate_summary_report(verbose: bool = True):
    ...
    report_lines.append("  4) L → C → D: 외로움 → 인지기능 → 우울")
    report_lines.append("")
...
def run(analysis: Optional[str] = None, verbose: bool = True, **kwargs) -> Dict[str, Any]:
    ...
    if analysis:
        ...
        elif analysis == 'gender_indirect_comparison':
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Path Model Comparison Suite (Depression)')
