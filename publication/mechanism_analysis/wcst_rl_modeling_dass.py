"""
WCST Reinforcement Learning Modeling (DASS-Controlled)
=====================================================
...
Usage:
    ...
    python -m publication.mechanism_analysis -a wcst_rl_modeling_dass --sub ucla_relationship

    from publication.mechanism_analysis.wcst_rl_modeling_dass import run
    run('rescorla_wagner')
...
# Output directory
OUTPUT_DIR = BASE_OUTPUT / "wcst_rl_modeling"
...
def load_wcst_trials() -> pd.DataFrame:
    ...
    if 'chosencard' in trials.columns:
        card_map = {card: idx for idx, card in enumerate(trials['chosencard'].dropna().unique())}
...
def run(analysis: Optional[str] = None, verbose: bool = True) -> Dict[str, pd.DataFrame]:
    ...
    if verbose:
        print("\n" + "=" * 70)
        print("RL MODELING SUITE COMPLETE")
...
def list_analyses():
    ...
    print("\nAvailable RL Modeling Analyses:")
    print("-" * 60)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="WCST Reinforcement Learning Modeling (DASS-Controlled)")
