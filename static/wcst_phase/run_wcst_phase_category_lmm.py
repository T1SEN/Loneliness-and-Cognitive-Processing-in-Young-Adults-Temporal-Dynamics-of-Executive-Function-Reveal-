...
def fit_with_fallback(df: pd.DataFrame, formula: str) -> tuple[object, dict[str, object], str | None, list[str]]:
    ...
    if last_result is None:
        raise RuntimeError(f"Phase-category LMM failed: {last_error}")
...
def main(confirm_len: int) -> None:
    ...
    df["category_num_c"] = df["category_num"] - df["category_num"].mean()

    formula = (
        "log_rt ~ C(phase) * category_num_c * z_ucla_score + "
    ...
    meta_path = output_dir / f"wcst_phase_category_lmm_{suffix}_meta.csv"
    pd.DataFrame([meta]).to_csv(meta_path, index=False, encoding="utf-8-sig")

    coef_df = _summarize_terms(result)
    coef_path = output_dir / f"wcst_phase_category_lmm_{suffix}_coeffs.csv"
