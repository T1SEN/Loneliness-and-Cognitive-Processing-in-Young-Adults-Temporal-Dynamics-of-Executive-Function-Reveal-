...
def fit_with_fallback(df: pd.DataFrame, formula: str) -> tuple[object, dict[str, object], str | None, list[str]]:
    attempts = [
    ...
    if last_result is None:
        raise RuntimeError(f"Phase-progress LMM failed: {last_error}")
...
def _scale_within(series: pd.Series) -> pd.Series:
...
def main(confirm_len: int) -> None:
    output_dir = OUTPUT_STATS_DIR / "analysis" / "overall" / "wcst_phase"
    ...
    wcst = wcst[wcst["rt_ms"].notna() & wcst[trial_col].notna()]
    ...
    wcst = wcst.dropna(subset=["log_rt", "phase"])

    wcst["trial_progress"] = wcst.groupby("participant_id")[trial_col].transform(_scale_within)
    wcst = wcst.dropna(subset=["trial_progress"])
    ...
    formula = (
        "log_rt ~ C(phase) * trial_progress * z_ucla_score + "
        "z_dass_depression + z_dass_anxiety + z_dass_stress + z_age + gender_male"
    )

    result, attempt, fit_note, warnings_list = fit_with_fallback(df, formula)
    ...
    meta_path = output_dir / f"wcst_phase_progress_lmm_{suffix}_meta.csv"
    pd.DataFrame([meta]).to_csv(meta_path, index=False, encoding="utf-8-sig")

    coef_df = _summarize_terms(result)
    coef_path = output_dir / f"wcst_phase_progress_lmm_{suffix}_coeffs.csv"
